...
class NetworkMock:
    ...
    def visualize(self, filepath=None, render=False, visualizer="networkx"):
        if visualizer == "yfiles":
            yfiles_vis = vis_yfiles.YFilesVisualizer(self.network)
            yfiles_vis.visualize(graph_layout="hierarchical", directed=True)
        elif visualizer == "networkx":
            networkx_vis = vis_networkx.NetworkXVisualizer(self.network)
            networkx_vis.visualize(render=render)
        else:
